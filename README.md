# Steam Game Patcher — Electron

An Electron + React + TypeScript rewrite of the original Python/customtkinter Steam Game Patcher.

It scans your installed Steam games, cross-references them against the [Nukige group's](https://steamcommunity.com/groups/Nukige) curated patch database, and helps you download and apply the patches available for your library.

> **Behaviour change from the Python version:** auto-installing patches is now **opt-in**. By default the app only downloads patch files and opens the relevant folders for you to apply them yourself. See [Beta Auto-Install](#beta-auto-install) below.
...
### Where things map back to the Python original

Most main-process modules are direct ports and name their Python counterpart in a header comment — e.g. `steam/discovery.ts` ← `get_steam_path()`, `patch/smartApply.ts` ← `smart_apply_patch()`, `database/flatten.ts` ← `flatten_game_contents()`. Start there when comparing behaviour against the old app.
...
5. **Update detection** — each game's `patcher_config.json` records the last applied patch filename. If that filename is no longer in the DB's current file list for that game, an **Update** badge appears.
   ...
   ⚠️ This is filename-based, inherited from the original. If a patch is re-uploaded under the *same* filename with different contents, it won't be detected as an update.
...
- **On** — archives also get an "Install patch" button that extracts and writes into the game folder using the smart-matching logic ported from the Python version (exact single filename match → overwrite in place; multiple matches → skip for safety; no match → add at the archive's relative path).

Either way, **nothing is written into a game folder or executed without a confirmation dialog** naming the exact game, install path, and files involved.
...
Unlike the Python version, which wrote to a `data/` folder next to the script, this app uses Electron's per-OS `userData` directory (Program Files isn't writable, macOS bundles are read-only):

| | Path |
|---|---|
| Windows | `%APPDATA%\steam-game-patcher\data\` |
| Linux | `~/.config/steam-game-patcher/data/` |
| macOS | `~/Library/Application Support/steam-game-patcher/data/` |

Contains `patches_database.json`, its `.etag`, `favorites.json`, `patcher.log`, and `cache/` (downloaded patches — relocatable in Settings).

**Per-game state stays in the game's own install folder** as `patcher_config.json`, with the same filename and shape as the Python version, so both apps read and write the same file. A one-time migration folds an old global `data/last_applied.json` into per-game configs if one is found.
...
To publish: tag a release (e.g. `v2.1.0`) and attach the installer from `dist/`.
