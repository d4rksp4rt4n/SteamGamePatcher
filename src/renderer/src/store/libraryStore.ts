 ...
 *   - pure UI state (view mode, search text, selected game) the Python version kept as
 *     instance attributes on the Tk `App` (`self.view_mode`, `self.search_var`, `self.current_appid`)
 */
...
export const useLibraryStore = create<LibraryStore>((set, get) => ({
  ...
  setLibrary: (result) =>
    set({
      ...
      // Auto-select the first game once data lands, mirroring the Python version
      // selecting `tree.get_children()[0]` right after `_build_gui()`.
