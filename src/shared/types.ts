 ...
 * Mirrors the data shapes from the original Python app (SteamGamePatcher.py v1.4.1-modern):
 *   - `installed games` map            -> get_installed_games()
 *   - `PatchFile` / `flatten_game_contents()` -> flat file list per game entry
 *   - `GameMatch` / `self.matches` + `self.by_id` -> cross-reference of DB entries vs installed games
 *   - `PerGameConfig` / `patcher_config.json` -> save_per_game_config()
 */
...
/** Map of appid -> InstalledGame, equivalent to Python's `self.installed` dict. */
...
/** A DB entry cross-referenced against an installed game, equivalent to Python's `self.matches` items. */
...
/** appid -> { gameName -> LastPatchRecord }, mirrors Python's `self.last_applied`. */
export type LastAppliedMap = Record<string, Record<string, LastPatchRecord>>

/** Result of the phase-1 pipeline (steam discovery -> db fetch -> matching -> favorites/config),
