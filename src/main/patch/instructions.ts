/**
 * Backs the in-app instructions viewer. Python's version opened these in a native
 * full-screen Tk dialog with real DOCX/PDF rendering (via python-docx and PyMuPDF); the
 * Electron equivalent renders inside the app window instead of shelling out to the OS's
 * default handler, for .txt and .docx. PDF isn't rendered in-app yet — a proper PDF
 * viewer is a much heavier dependency (pdfjs-dist) and is a reasonable follow-up rather
 * than blocking this pass; PDFs still open via the OS default viewer for now.
 */
