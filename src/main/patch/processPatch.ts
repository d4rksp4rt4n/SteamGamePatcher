/**
 * Port of `App.process_patch()` from SteamGamePatcher.py, split along the line the brief
 * requires: everything through "a valid file sits in the cache folder" is common to both
 * modes; what happens after that branches hard on the Beta Auto-Install setting.
 *
 ...
 *
 *   Beta ON: extract (or silently run, for .exe) each non-instruction file into a temp
 *   dir, then smart-apply it into the install dir, then persist the per-game config —
 *   this is the old always-on Python behavior, now opt-in and pre-confirmed by the caller.
 */
...
export interface ProcessPatchResult {
   ...
   *  (matches the Python `applied_file` variable used for the config save). */
