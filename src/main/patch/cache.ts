/**
 * Port of the cache-reuse block at the top of `App.process_patch()`'s per-file loop:
 * reuse a cached download if it passes a size-tolerance check and a 7z integrity test,
 * otherwise (re-)download with up to 3 retries, deleting and retrying on any failure.
 */
...
/** True if `actual` bytes is close enough to `expected` to trust, replicating Python's
 *  "small file OR within 5% tolerance" rule (small files' size strings are often rounded
 *  hard enough that a strict tolerance check would reject a perfectly good download). */
 ...
 * on disk, or throws after 3 failed attempts — same contract as the Python version.
 */
async function obtainFileInner(params: ObtainFileParams): Promise<string> {
  ...
  if (existsSync(cachePath)) {
    ...
    if (sizeIsAcceptable(actual, expected)) {
      ...
      // check alone is enough to trust the cache, exactly like Python's `local_7z.exists()`
      // gate (which only ever ran `7z t` on non-exe cache hits in the first place).
