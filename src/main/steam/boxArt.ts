/**
 * Port of `load_box_art_pil()` from SteamGamePatcher.py — candidate discovery and
 * "best match" selection only. The Python version went on to open the image with PIL,
 * thumbnail it, and paste it onto a 200x300 canvas; in Electron that compositing is
 * unnecessary and slower than just letting an <img> with `object-fit: cover` do it in
 * CSS, so this module's job ends at "here is the best file path on disk for this appid".
 *
 * Search order (identical to Python):
