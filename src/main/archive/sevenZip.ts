/**
 * Port of `App.extract_with_7z()` / `App.extract_archive()` from SteamGamePatcher.py,
 * plus the ad-hoc `7z t` integrity-test calls sprinkled through `process_patch()`.
 * Uses `7zip-bin`'s bundled binaries instead of shelling out to a `7z.exe` the Python
 * version expected to find next to the script — this is what makes the archive pipeline
