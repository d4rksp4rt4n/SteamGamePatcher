/**
 * Port of the "BUILD MATCHES + DEEP DEBUG" block inside `App.__init__`.
 *
 * Cross-references every DB entry's appid against the locally installed Steam games,
 * producing the same two structures the Python version kept: a sorted list of matches
 * (`self.matches`) and an appid-keyed lookup (`self.by_id`).
 */
 ...
 * This is a straight port of the tuple-key sort the Python UI applied when populating
 * the Treeview/grid (favorite + hasUpdate > hasUpdate > favorite > normal).
 */
