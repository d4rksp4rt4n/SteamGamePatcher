/**
 * Node has no direct equivalent of Python's `gdown` package, and gdown itself has had to
 * repeatedly adapt to how Google's "can't scan this file for viruses" interstitial works —
 * it's an actively-moving target, not a stable API. As of 2026 the flow for a file too
 * large to virus-scan is:
 *
...
export interface DownloadProgress {
  /** 0-100, or -1 if total size is unknown (mirrors Python's progress_bar=-1 "indeterminate"). */
  percent: number
 ...
 * (process_patch.ts loops up to 3 attempts, same as Python).
 */
